#this algorithm works, reward > 200(e.g:Total Reward: 292.89)
#Loading best model and testing...
#Test Episode 1: Total Reward = 297.76
#Test Episode 2: Total Reward = 272.90
#Test Episode 3: Total Reward = 287.00
#Test Episode 4: Total Reward = 59.30
#Test Episode 5: Total Reward = 255.17
#Average Test Reward: 234.43
...
# 创建保存模型的目录
if not os.path.exists('sac_lunar_lander'):
    os.makedirs('sac_lunar_lander')
...
class SACAgent:
    def __init__(self, state_dim, action_dim, lr=3e-4, gamma=0.99, tau=0.005, alpha=0.2,
                 buffer_size=100000, batch_size=256, auto_entropy_tuning=True):
        ...
        if self.auto_entropy_tuning:
            ...
            # Initialize self.alpha from log_alpha
            self.alpha = self.log_alpha.exp().item()  # Or use .detach() if needed as tensor
        else:
            self.alpha = alpha  # Make sure the parameter 'alpha' is passed to __init__
    ...
    def save_models(self, episode):
        torch.save(self.actor.state_dict(), f'sac_lunar_lander/actor_{episode}.pth')
        torch.save(self.critic.state_dict(), f'sac_lunar_lander/critic_{episode}.pth')
        torch.save(self.critic_target.state_dict(), f'sac_lunar_lander/critic_target_{episode}.pth')

    def load_models(self, episode):
        self.actor.load_state_dict(torch.load(f'sac_lunar_lander/actor_{episode}.pth'))
        self.critic.load_state_dict(torch.load(f'sac_lunar_lander/critic_{episode}.pth'))
        self.critic_target.load_state_dict(torch.load(f'sac_lunar_lander/critic_target_{episode}.pth'))


# 训练函数
def train_sac(env_name="LunarLander-v3", num_episodes=1000, max_steps=1000, continuous=True):
    # 创建环境
    if continuous:
    ...
    for episode in range(num_episodes):
        ...
        for step in range(max_steps):
            ...
            # 对于离散动作空间，需要将连续动作转换为离散动作
            if not continuous:
                # 将连续动作转换为离散动作
                action_discrete = np.argmax(action) if len(action) > 1 else int(action[0] > 0)
                next_state, reward, done, truncated, _ = env.step(action_discrete)
            else:
...
# 测试训练好的Agent
def test_agent(agent, env_name="LunarLander-v3", num_episodes=5, render=True, continuous=True):
    if continuous:
        env = gym.make(env_name, continuous=True, render_mode="human" if render else None)
    else:
    ...
    for episode in range(num_episodes):
        ...
        while not (done or truncated):
            ...
            if not continuous:
                action_discrete = np.argmax(action) if len(action) > 1 else int(action[0] > 0)
                state, reward, done, truncated, _ = env.step(action_discrete)
            else:
...
def plot_training_results(rewards, critic_losses, actor_losses, alpha_losses, lengths):
    ...
    plt.tight_layout()
    plt.savefig('sac_lunar_lander_training_results.png')
    plt.show()


# 主函数
if __name__ == "__main__":
    print("Starting SAC training on LunarLander...")

    # 选择是否使用连续动作空间
    use_continuous = True  # 设置为False使用离散动作空间
    ...
    test_agent(agent, "LunarLander-v3", num_episodes=5, render=True, continuous=use_continuous)